...
function [reduced_data, new_dims] = reduce_data_ModelFit(data, dims)
...
%     n_time_pts   = size(data, 2);
    ...
    %% Average across factors not involved in this effect
       %Changes were made so that the test is across the 1st dimension
       %rather than the 1st two dimensions
       
    if (length(wdims) < ndims(data) - 2) || isempty(wdims)
        %Put the factors to average across as the initial dimensions
        num_dims = ndims(data);
        all_dims = 1:ndims(data);
        reorder = [all_dims(~ismember(all_dims, [1,wdims,num_dims])),...
            all_dims(ismember(all_dims, [1,wdims,num_dims]))];
        ...
        num_dims_to_avg = ndims(data) - length(wdims) - 2;
        reduced_data = reshape(reduced_data, [prod(dim_sizes(1:num_dims_to_avg)),...
            dim_sizes((num_dims_to_avg+1):end)]);
    ...
    dim_sizes = size(reduced_data);
    factor_levels = dim_sizes(2:(ndims(reduced_data)-1));
    if length(dims)>1 && sum(factor_levels==2)
        %Put the factors to subtract across as the initial dimensions
        if sum(factor_levels>2)
            reorder = [find(factor_levels==2)+1, 1, find(factor_levels~=2)+1, ndims(reduced_data)];
        elseif bg
%             reorder = [3:(ndims(reduced_data)-1), 1, 2, ndims(reduced_data)];   
            reorder = [2:(ndims(reduced_data)-1), 1, ndims(reduced_data)];
        else
%             reorder = [4:(ndims(reduced_data)-1), 1, 2, 3, ndims(reduced_data)];
        ...
        if sum(factor_levels>2)
            end_size = n_electrodes * prod(factor_levels(factor_levels~=2)) * n_subs;
        elseif bg
            end_size = n_electrodes * n_subs;
        else
            end_size = n_electrodes * 2 * n_subs;
        end
        ...
        if sum(factor_levels>2)
            reduced_data = reshape(reduced_data, [n_electrodes, factor_levels(factor_levels~=2), size(data,ndims(data))]);
        elseif bg
            reduced_data = reshape(reduced_data, [n_electrodes, n_subs]);
        else
            reduced_data = reshape(reduced_data, [n_electrodes, 2, n_subs]);
        end
    end
    
    %% Dimensions of reduced_data involved in effect
    
    new_dims = (2:ndims(reduced_data)-1);
    if bg
