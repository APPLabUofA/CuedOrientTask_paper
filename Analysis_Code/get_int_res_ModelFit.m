...
function int_res = get_int_res_ModelFit(data, cond_subs, dims)
        ...
        for q = 1:n_conds_B
            for r = 1:n_conds_C
                int_res(:,p,q,r,:) = data(:,p,q,r,:) ... 
                                        - mean(mean(data(:,p,:,:,:), 3), 4) ...
                                        - mean(mean(data(:,:,q,:,:), 2), 4) ...
                                        - mean(mean(data(:,:,:,r,:), 2), 3) ...
                                        + 2*mean(mean(mean(data, 2), 3), 4);
            end
        end
    end
end
            ...
            for r = 1:n_conds_C
                for n = 1:cond_subs(p)
                    ...
                    int_res(:, q, r, sub) = data(:, q, r, sub) ...
                                                - mean(mean(data(:, q, :, :), 3), 4) ...
                                                - mean(mean(data(:, :, r, :), 2), 4) ...
                                                - mean(mean(data(:, :, :, sub), 2), 3) ...
                                                + 2*mean(mean(mean(data, 2), 3), 4);
                end
            end
        end
