...
%
function model = VariablePrecisionModel_GammaPrecision_nog()
  model.name = 'Variable precision model (gamma over precision)';
	model.paramNames = {'modePrecision', 'sdPrecision'};
	model.lowerbound = [0.001 0.001]; % Lower bounds for the parameters
	model.upperbound = [100 100]; % Upper bounds for the parameters
	model.movestd = [0.05, 0.05];
	model.pdf = @vp_pdf;
  model.modelPlot = @model_plot;
	model.start = [0.001, 0.1;
                 0.01, 0.2;
                 0.002, 0.05;
                 0.002, 0.1];
  ...
  function y = vp_pdf(data,modePrecision,sdPrecision)
      g=0;
...
%     probDataUnderThisNormal = (1-g).*model.v + (g).*1/360;
  ...
  function figHand = model_plot(data, params, varargin)
    ...
    for i=1:size(params,1)
      modePrecision = params(i,1);
      sdPrecision = params(i,2);
      scale = (2*sdPrecision^2) / (modePrecision+sqrt(modePrecision^2+4*sdPrecision^2));
      shape = 1 + modePrecision*(1/scale);
      y = gampdf(precisionsSumOver, shape, scale);
      plot(x, y, 'Color', [0.54, 0.61, 0.06]); hold on;
    end
    modePrecision = maxParams(1);
    sdPrecision = maxParams(2);
    scale = (2*sdPrecision^2) / (modePrecision+sqrt(modePrecision^2+4*sdPrecision^2));
    shape = 1 + modePrecision*(1/scale);
    y = gampdf(precisionsSumOver, shape, scale);
