...
function test_results = calc_Fmax_ModelFit(data, cond_subs, dims, n_perm, alpha)
    ...
    if ~isempty(cond_subs) && ~isequal(cond_subs, 0) && length(cond_subs) > 1
        ...
        [F_obs, F_dist, df_effect, df_res, exact_test] = perm_rbANOVA_OrientExo(data, dims, n_perm);
    end
    
    %%% Calculate Fmax correction %%%
    
    [h, p, Fmax_crit, est_alpha] = Fmax_corr_ModelFit(F_obs, F_dist, alpha);
