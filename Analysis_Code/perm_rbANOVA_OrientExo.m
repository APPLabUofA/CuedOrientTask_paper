...
function [F_obs, F_dist, df_effect, df_res, exact_test] = perm_rbANOVA_OrientExo...
    (data, dims, n_perm, reduce)
    ...
    if reduce
        reduced_data = reduce_data_ModelFit(data, dims);
    else
        reduced_data = data;
    end
...
%Perform permutation one-way ANOVA. This is an exact test.
    ...
    %Some useful numbers
    [n_electrodes, n_conds, n_subs] = size(data);
    ...
    F_dist = NaN(n_perm, n_electrodes);
    for i = 1:n_perm
        ...
        if i ==1
            ...
            for n = 1:n_subs
                perm_data(:, :, n) = data(:, randperm(size(data, 2)), n);
            end
        end
        
        %Calculate sums of squares
        SSyint = (sum(sum(perm_data, 2), 3).^2) / (n_conds * n_subs);
       %SSTO   = sum(sum(perm_data.^2, 3), 4) - SSyint;
        SSA    = (sum(sum(perm_data, 3).^2, 2) / n_subs) - SSyint;
        SSBL   = (sum(sum(perm_data, 2).^2, 3) / n_conds) - SSyint;
        SSRES  = sum(sum(perm_data.^2, 2), 3) - SSA - SSBL - SSyint;
    ...
    F_obs = F_dist(1, :);
%     F_obs = reshape(F_dist(1, :), n_electrodes);
...
%two-way interaction.
    ...
    flat_data = reshape(int_res, [n_electrodes, n_conds_A*n_conds_B, n_subs]);
    ...
    for i = 1:n_perm
        ...
        if i == 1
            ...
            for s = 1:n_subs
                flat_perm_data(:, :, s) = flat_data(:, randperm(size(flat_data, 2)), s);
            end
            perm_data = reshape(flat_perm_data, n_electrodes, n_conds_A, n_conds_B, n_subs);
        end
        
        %Calculate sums of squares
        SSyint   = (sum(sum(sum(perm_data, 2), 3), 4).^2)/(n_conds_A*n_conds_B*n_subs);
        %SSTO     = sum(sum(sum(perm_data.^2, 3), 4), 5) - SSyint;
        SSA      = sum(sum(sum(perm_data, 3), 4).^2, 2)/(n_conds_B*n_subs) - SSyint;
        SSB      = sum(sum(sum(perm_data, 2), 4).^2, 3)/(n_conds_A*n_subs) - SSyint;
        SSBL     = sum(sum(sum(perm_data, 2), 3).^2, 4)/(n_conds_A*n_conds_B) - SSyint;
        SSAxB    = sum(sum(sum(perm_data, 4).^2, 2), 3)/n_subs - SSA - SSB -SSyint;
        SSAxBL   = sum(sum(sum(perm_data, 3).^2, 2), 4)/n_conds_B - SSA - SSBL - SSyint;
        SSBxBL   = sum(sum(sum(perm_data, 2).^2, 3), 4)/n_conds_A - SSB - SSBL - SSyint;
        SSAxBxBL = sum(sum(sum(perm_data.^2, 2), 3), 4) - SSA - SSB - SSBL - SSAxB - SSAxBL - SSBxBL - SSyint;
...
%three-way interaction.
    ...
    %Some useful numbers
    [n_electrodes, n_conds_A, n_conds_B, n_conds_C, n_subs] = size(data);
    ...
    %interaction
    int_res = get_int_res_ModelFit(data, [], [2, 3, 4]);
   ...
   %dfRES      = (n_subs - 1) * (n_conds_A * n_conds_B * n_conds_C - 1);
    ...
    %Re-arrange data for permutation
    flat_data = reshape(int_res, [n_electrodes, n_conds_A*n_conds_B*n_conds_C, n_subs]);
    ...
    for i = 1:n_perm
        ...
        %Calculate sums of squares
        SSyint     = (sum(sum(sum(sum(perm_data, 2), 3), 4), 5).^2)/(n_conds_A*n_conds_B*n_conds_C*n_subs);
       %SSTO       = sum(sum(sum(sum(perm_data.^2, 3), 4), 5), 6) - SSyint;
        SSA        = sum(sum(sum(sum(perm_data, 3), 4), 5).^2, 2)/(n_conds_B*n_conds_C*n_subs) - SSyint;
        SSB        = sum(sum(sum(sum(perm_data, 2), 4), 5).^2, 3)/(n_conds_A*n_conds_C*n_subs) - SSyint;
        SSC        = sum(sum(sum(sum(perm_data, 2), 3), 5).^2, 4)/(n_conds_A*n_conds_B*n_subs) - SSyint;
        SSBL       = sum(sum(sum(sum(perm_data, 2), 3), 4).^2, 5)/(n_conds_A*n_conds_B*n_conds_C) - SSyint;
        SSAxB      = sum(sum(sum(sum(perm_data, 4), 5).^2, 2), 3)/(n_conds_C*n_subs) - SSA - SSB -SSyint;
        SSAxC      = sum(sum(sum(sum(perm_data, 3), 5).^2, 2), 4)/(n_conds_B*n_subs) - SSA - SSC -SSyint;
        SSBxC      = sum(sum(sum(sum(perm_data, 2), 5).^2, 3), 4)/(n_conds_A*n_subs) - SSB - SSC -SSyint;
        SSAxBxC    = sum(sum(sum(sum(perm_data, 5).^2, 2), 3), 4)/n_subs - SSA - SSB - SSC - SSAxB - SSAxC - SSBxC - SSyint;
        SSAxBL     = sum(sum(sum(sum(perm_data, 3), 4).^2, 2), 5)/(n_conds_B*n_conds_C) - SSA - SSBL - SSyint;
        SSBxBL     = sum(sum(sum(sum(perm_data, 2), 4).^2, 3), 5)/(n_conds_A*n_conds_C) - SSB - SSBL - SSyint;
        SSCxBL     = sum(sum(sum(sum(perm_data, 2), 3).^2, 4), 5)/(n_conds_A*n_conds_B) - SSC - SSBL - SSyint;
        SSAxBxBL   = sum(sum(sum(sum(perm_data, 4).^2, 2), 3), 5)/n_conds_C - SSA - SSB - SSBL - SSAxB - SSAxBL - SSBxBL - SSyint;
        SSAxCxBL   = sum(sum(sum(sum(perm_data, 3).^2, 2), 4), 5)/n_conds_B - SSA - SSC - SSBL - SSAxC - SSAxBL - SSCxBL - SSyint;
        SSBxCxBL   = sum(sum(sum(sum(perm_data, 2).^2, 3), 4), 5)/n_conds_A - SSB - SSC - SSBL - SSBxC - SSBxBL - SSCxBL - SSyint;
        SSAxBxCxBL = sum(sum(sum(sum(perm_data.^2, 2), 3), 4), 5) - SSA - SSB - SSC - SSBL - SSAxB - SSAxC - SSBxC - SSAxBL - SSBxBL - SSCxBL - SSAxBxC - SSAxBxBL - SSAxCxBL - SSBxCxBL - SSyint;
