...
function model = Stochastic_Model()
  ...
  model.pdf = @(data, kappa, xi) (pNeuralModel(wrap(data.errors(:)/180*pi), kappa, xi));
%   model.pdf = @(dx, kappa, xi) -sum(log(pNeuralModel(dx', kappa, xi)));
%   @pNeuralModel(dx, kappa, xi;
...
function y = modelpNM(data, kappa, xi)
    data.errors = wrap(data.errors/180*pi);
    y = pNeuralModel(data.errors(:),kappa,xi);
end
